...
class LayerUtils:
    ...
    def detectColumnType(resultmap):
        ...
        if intcount == len(resultmap):
            return {"type": QVariant.Int, "xsdtype": "xsd:integer", "unique": (stringcount == len(uniquestrings))}
        if doublecount == len(resultmap):
            return {"type": QVariant.Double, "xsdtype": "xsd:double", "unique": (stringcount == len(uniquestrings))}
        if datecount == len(resultmap):
            return {"type": "xsd:date", "xsdtype": "xsd:date", "unique": (stringcount == len(uniquestrings))}
        if uricount == len(resultmap):
           return {"type": "xsd:anyURI", "xsdtype": "xsd:anyURI", "unique": (stringcount == len(uniquestrings))}
        return {"type":QVariant.String,"xsdtype":"xsd:string","unique":(stringcount==len(uniquestrings))}
    ...
    def detectLayerColumnType(layer,columnindex):
        ...
        for feat in features:
            ...
            QgsMessageLog.logMessage(str(attrs[0]),
